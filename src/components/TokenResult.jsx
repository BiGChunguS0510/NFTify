...
import "./TokenResult.css";

export const TokenResult = ({result}) => {

  const splitString = (inputString) => {
    const length = inputString.length;
    if(length > 60) inputString = inputString.slice(0, 65);
    const partSize = Math.ceil(length / 3);
    const part1 = inputString.slice(0, partSize);
    const part2 = inputString.slice(partSize, 2 * partSize);
    const part3 = inputString.slice(2 * partSize);

    return [part1, part2, part3];
  }

  const {pairCreatedAt, pairAddress, dexId, baseToken, quoteToken, priceNative, priceUsd} = result;
  const pa = splitString(pairAddress);
  const ba = splitString(baseToken.address);
  const qa = splitString(quoteToken.address);
  return (
    <div className="token-result-container">
      ...
      <div className="info-card">
        ...
        <div className="info-cols">
          <div className="info-labels">
            <div className="info-row">Pair Created At</div>
            <div className="info-row">Dex ID</div>
            <div className="info-row">Pair Address</div>
          </div>
          <div className="info-values">
            <div className="info-row">{pairCreatedAt}</div>
            <div className="info-row">{dexId}</div>
            <div className="info-row"><div className="address-div">
              <div>{pa[0]}</div>
              <div>{pa[1]}</div>
              <div>{pa[2]}</div>
            </div></div>
      ...
      <div className="info-card">
        <div className="info-header">Base Token</div>
        <div className="info-cols">
          <div className="info-labels">
            <div className="info-row">Name</div>
            <div className="info-row">Symbol</div>
            <div className="info-row">Address</div>
          </div>
          <div className="info-values base-values">
            <div className="info-row">{baseToken.name}</div>
            <div className="info-row">{baseToken.symbol}</div>
            <div className="info-row"><div className="address-div">
              <div>{ba[0]}</div>
              <div>{ba[1]}</div>
              <div>{ba[2]}</div>
      ...
      <div className="info-card">
        <div className="info-header">Quote Token</div>
        <div className="info-cols">
          <div className="info-labels">
            <div className="info-row">Name</div>
            <div className="info-row">Symbol</div>
          ...
          <div className="info-values base-values">
            <div className="info-row">{quoteToken.name}</div>
            <div className="info-row">{quoteToken.symbol}</div>
            <div className="info-row"><div className="address-div">
              <div>{qa[0]}</div>
              <div>{qa[1]}</div>
              <div>{qa[2]}</div>
      ...
      <div className="info-card">
        ...
        <div className="info-cols">
          <div className="info-labels">
            <div className="info-row">Price Native</div>
            <div className="info-row">Price USD</div>
          </div>
          <div className="info-values">
            <div className="info-row">{priceNative}</div>
            <div className="info-row">{priceUsd}</div>
          </div>
        </div>
        <div className="info-icon">
          <svg
